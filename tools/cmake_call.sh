...
# so each object is defined exactly once; the script's own guard fails the build
# if a SUNDIALS upgrade introduces a bundling it does not know about.
#
...
if [ $? -ne 0 ]; then
    echo "De-duplicating SUNDIALS static libraries failed!"
    exit 1
fi
