...
- jacobian:
  ...
  (Optional) Jacobian with signature `function(t, y, ydot, cj, p)`
  returning an n-by-n matrix of `dF/dy + cj*dF/dydot`. Default NULL.
...
## DAEs can also be described using Rcpp. Compiling the C++ version invokes the
