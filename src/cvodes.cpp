...
NumericMatrix cvodes(NumericVector time_vector, NumericVector IC, SEXP input_function,
  ...
  case CLOSXP:{
    ...
    if (check_retval(&flag, "CVodeSetEwtFn", 1)) return(1);

    /* Create dense SUNMatrix */
    sunindextype y_len_M = y_len;
    SUNMatrix SM = SUNDenseMatrix(y_len_M, y_len_M);
    if (check_retval((void *)SM, "SUNDenseMatrix", 0)) return(1);
    ...
    if (check_retval(&flag, "CVodeSetLinearSolver", 1)) return(1);
    ...
    if(check_retval(&flag, "CVodeSensInit", 1)) return(1);

    /* Call CVodeSensEEtolerances to estimate tolerances for sensitivity
     variables based on the rolerances supplied for states variables and
     the scaling factor pbar */
    flag = CVodeSensEEtolerances(cvode_mem);
    if(check_retval(&flag, "CVodeSensEEtolerances", 1)) return(1);
    ...
    Rcout << &(ptr->params[0]);
    flag = CVodeSetSensParams(cvode_mem, &(ptr->params[0]), Parameters.begin(), NULL);  // double *y = x.begin()
    if (check_retval(&flag, "CVodeSetSensParams", 1)) return(1);
    ...
    for(int iout = 0; iout < NOUT-1; iout++) {
      ...
      if (check_retval(&flag, "CVodeGetSens", 1)) { break; }
