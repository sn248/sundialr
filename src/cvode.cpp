...
// int rhs_xptr(realtype t, N_Vector y, N_Vector ydot, void* user_data){
//
...
//   struct rhs_xptr *my_rhs_ptr = (struct rhs_xptr*)user_data;
...
NumericMatrix cvode(NumericVector time_vector, NumericVector IC, SEXP input_function,
  ...
  case CLOSXP:{
    ...
    struct rhs_func my_rhs = {input_function};
    ...
    flag = CVodeInit(cvode_mem, rhs_func, T0, y0);
  ...
  // case EXTPTRSXP: {
  //
  //   struct rhs_xptr my_rhs = {input_function};
  //   // setting the user_data in rhs function
  //   flag = CVodeSetUserData(cvode_mem, (void*)&my_rhs);
  //   if (check_flag(&flag, "CVodeSetUserData", 1)) { stop("Stopping cvode!"); }
  //
  //   flag = CVodeInit(cvode_mem, rhs_xptr, T0, y0);
  //   if (check_flag(&flag, "CVodeInit", 1)) { stop("Stopping cvode!"); }
  //
  //   break;
  // }
  ...
  default: {
    stop("Incorrect input function type - input function can be an R function or an expternal pointer to Rcpp function");
    }
